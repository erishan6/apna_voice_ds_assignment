...
def evaluate(annotations, detections, tolerance=.2, manual_annotation = False):
    ...
    for turn_true in true_turns:
        ...
        if abs(closest_detection - turn_true) <= tolerance:
            tp += 1
...
with open("task1_VAD_test_timestamps.csv", "w+") as f:
    # f.write()
    f.write("Start, End\n")
...
df.to_csv("task1_vad_metrics.csv")
